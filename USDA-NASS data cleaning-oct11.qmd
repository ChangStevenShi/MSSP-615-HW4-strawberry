...
# Convert Value column to numeric after handling non-numeric characters
plot1_data$Value <- as.numeric(gsub("[^0-9]", "", plot1_data$Value))

# Arrange data in descending order of Value
plot1_data <- plot1_data %>%
  arrange(desc(Value))

# Create and optimize the plot
ggplot(plot1_data, aes(x=reorder(State, -Value), y=Value)) + 
  geom_bar(stat = "identity", fill = "#FF9999", color = "#FF6666") + 
  theme_minimal() +
...
# Adjusting the code based on the correct column names
plot2_data <- strawberry %>%
  select(Year, State, `Data Item`, Domain, Value) %>%
  filter((Year == 2021) & 
...
# Convert Value column to numeric after handling non-numeric characters
plot2_data$Value <- as.numeric(gsub("[^0-9]", "", plot2_data$Value))
...
ggplot(plot2_data, aes(x=reorder(State, -Value), y=Value)) + 
  geom_bar(stat = "identity", fill = "#66CCFF", color = "#3399FF") + 
  theme_minimal() +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=10),
        axis.text.y=element_text(size=10),
        title=element_text(size=14, face="bold")) +
...
# View the structure of strwb_survey dataframe
glimpse(strwb_survey)

# Extract distinct values from various columns of strwb_survey
c3 <- distinct(strwb_survey[,3])
c4 <- distinct(strwb_survey[,4])
c6 <- distinct(strwb_survey[,6])
c7 <- distinct(strwb_survey[,7])
c8 <- distinct(strwb_survey[,8])
c5 <- distinct(strwb_survey[,5])

# Display the distinct values using kable for a better presentation
kable(c3, caption = "Distinct values from column 3")
kable(c4, caption = "Distinct values from column 4")
kable(c5, caption = "Distinct values from column 5")
kable(c6, caption = "Distinct values from column 6")
kable(c7, caption = "Distinct values from column 7")
kable(c8, caption = "Distinct values from column 8")
...
# Extract unique values from the Period column of strwb_survey
per_c <- strwb_survey %>%
  select(Period) %>%
  distinct() %>%
  pull()  # This converts the resulting tibble to a vector

# Display the distinct periods
print(per_c)

# Based on the provided descriptions, assign the descriptive names to the periods

period_descriptions <- c(
  "MARKETING YEAR" = "marketing year",
  "YEAR" = "year",
  "YEAR - AUG FORECAST" = "year - Aug Forecast"
)

# Match the periods in per_c to their descriptions
matched_periods <- period_descriptions[per_c]

# Display the matched periods with their descriptions
print(matched_periods)
...
# Since 'Data Item' column has already been split into multiple columns (temp1a, temp1b, temp2, temp3, temp4),
# we can directly work with these columns.

# Further split the 'temp1' column
#strwb_survey <- strwb_survey %>%
#  separate(temp1, 
#           into = c("temp1a", "temp1b"), 
#           sep = " - ", 
#           extra = "drop", 
#           fill = "right")

# Check the distinct values of 'temp1a' to confirm its content
a <- distinct(strwb_survey, temp1a)
distinct_domains <- distinct(strwb_survey, Domain)
print(distinct_domains)
# Examine the 'Domain' column to understand the different categories of rows
# Start by splitting it into multiple columns if necessary
# (You mentioned wanting to split it, but did not provide the exact code for this part)

# ... (Your code for splitting 'Domain' column if needed)
...
# Extract rows related to "CHEMICAL, HERBICIDE"
herbicide_data <- strwb_survey %>%
  filter(Domain == "CHEMICAL, HERBICIDE")

# Get distinct chemicals (herbicides) used
distinct_herbicides <- herbicide_data %>%
  distinct(temp2) %>%  # Assuming the specific herbicide names are in the 'temp2' column
  pull(temp2)

print(distinct_herbicides)

# Compare with WHO's list of carcinogenic chemicals
who_carcinogens <- c("capatafol", "dibromochloropropane", "glyphosate", 
                     "malathion", "atrazine", "dichlorodiphenyltrichloroethane")

common_chemicals <- intersect(tolower(distinct_herbicides), who_carcinogens)
...
library(dplyr)
# Assuming you've already loaded tidyr, dplyr, and ggplot2

# Filter the dataset for a specific metric (e.g., sales, production)
# Here, I am assuming the metric is in the 'Value' column and selecting data for the year 2021
time_series_data <- strawberry %>%
  filter(Year == 2021 & !is.na(Value) & Value != "(D)") %>%
  select(Year, Value)

# Convert the 'Value' column to numeric
time_series_data$Value <- as.numeric(gsub(",", "", time_series_data$Value))

# Plot the PACF
# Filter the dataset for a specific metric and remove NAs
time_series_data <- strawberry %>%
  filter(Year == 2021 & !is.na(Value) & Value != "(D)") %>%
  select(Year, Value) %>%
  filter(!is.na(Value))

time_series_data$Value[time_series_data$Value == "(D)"] <- 0
...
# Plot the PACF
pacf(time_series_data$Value, main="Partial Autocorrelation for Selected Metric")
